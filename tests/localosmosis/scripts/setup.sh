...
create_two_asset_pool () {
    ...
    while [ $COUNTER -lt 15 ]; do
        string=$(osmosisd tx gamm create-pool --pool-file=pools/$1 --from pools --chain-id=$CHAIN_ID --home $OSMOSIS_HOME --keyring-backend=test -b block --yes  2>&1)
...
create_three_asset_pool () {
    ...
    COUNTER=0
    while [ $COUNTER -lt 15 ]; do
        string=$(osmosisd tx gamm create-pool --pool-file=pools/nativeDenomThreeAssetPool.json --from pools --chain-id=$CHAIN_ID --home $OSMOSIS_HOME --keyring-backend=test -b block --yes 2>&1)
        if [ "$string" != "${string%"$substring"*}" ]; then
